...
题小鹰是一个微信小程序答题 MVP。当前项目以数学选择题练习为主入口，同时保留微信官方示例工程中的组件、接口、云开发和扩展能力分包，便于后续继续复用小程序能力样例。

## 项目分析

当前工程仍保留微信小程序示例项目的底座，但主业务已经切换到 `miniprogram/page/quiz/index`。应用启动后优先进入答题页，底部 tab 继续提供组件、扩展能力、接口和云开发示例入口。

答题业务目前是纯前端本地 MVP：题库写在页面脚本内，答题记录、错题回顾和分数计算在页面状态中完成，历史最佳成绩使用本地缓存 `quizBestScore` 保存。云开发能力已经初始化，并保留 `wxContext`、临时文件 URL、数据库读取和开放接口演示云函数，但答题主流程暂未依赖服务端。

## 功能

- 10 道数学选择题练习，支持 A/B/C/D 单选。
- 答题后即时显示正确/错误状态和题目解析。
